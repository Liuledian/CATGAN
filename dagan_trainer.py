...
class DaganTrainer:
    def __init__(
        ...
        self.regression = nn.Sequential(nn.Linear(28*28, 1024),nn.Linear(1024, self.g.z_dim)).cuda()
    ...
    def _generator_train_iteration(self, x1):
        ...
        # Get generated data
        #print('x1',x1.shape)
        z = torch.randn((x1.shape[0], self.g.z_dim)).cuda()
        #print('z',z.shape)
        generated_data = self.sample_generator(x1,z)
        #print('generated_data',generated_data.shape)

        # Calculate loss and optimize
        d_generated = self.d(x1, generated_data)

        re_generated=generated_data.view(x1.shape[0],-1)
        #print('re_generated',re_generated.shape)
        re_z=self.regression(re_generated)
        loss_r=self.l2_loss(re_z,z)
        g_loss = -d_generated.mean()+0.01*loss_r
        g_loss.backward()
        self.g_opt.step()

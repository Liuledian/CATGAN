...
def create_generated_batch(samples, generator, device="cpu"):
    ...
    with torch.no_grad():
        g_out = generator(samples.cuda(), z.cuda()).cpu()
...
def perform_train_step(
    ...
    for i, data in enumerate(train_dataloader):
        ...
        if i % real_batch_rate == 0:
            inputs = create_real_batch(inputs).cuda()
            flags = torch.ones((inputs.shape[0], 1)).cuda()
        else:
            inputs = create_generated_batch(inputs, g, device).cuda()
            flags = torch.zeros((inputs.shape[0], 1)).cuda()
